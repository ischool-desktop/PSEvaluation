...
namespace JHSchool.Evaluation.Ranking
{
    internal class SeparateTopReport : AbstractReport
    {
        ...
        public SeparateTopReport(List<RankData> rankData, Dictionary<string, JHStudentRecord> studentDict)
        {
            _rankData = rankData;
        ...
        public override void Export()
        {
            ...
            foreach (string name in printData.Keys)
            {
                foreach (string id in printData[name].Keys)
                {
                    ...
                    rowIndex++;
                    colIndex = 0;
                    ws.Cells[rowIndex, colIndex++].PutValue((student.Class != null) ? student.Class.Name : "");
