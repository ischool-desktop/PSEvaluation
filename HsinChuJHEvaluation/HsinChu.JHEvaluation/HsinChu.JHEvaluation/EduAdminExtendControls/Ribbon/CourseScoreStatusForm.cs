...
namespace HsinChu.JHEvaluation.EduAdminExtendControls.Ribbon
{
    public partial class CourseScoreStatusForm : FISCA.Presentation.Controls.BaseForm
    {
        ...
        private Dictionary<string, HC.JHAEIncludeRecord> _aeCache;
        ...
        public CourseScoreStatusForm()
        {
            ...
            _aeCache = new Dictionary<string, HC.JHAEIncludeRecord>();
        }
        ...
        private void SyncCacheManager()
        {
            ...
            foreach (JHAEIncludeRecord ae in JHAEInclude.SelectAll())
            {
                if (!_aeCache.ContainsKey(ae.ID))
                    _aeCache.Add(ae.ID, new HC.JHAEIncludeRecord(ae));
            }
        }
        ...
        {
            Dictionary<CourseRecord, HC.JHAEIncludeRecord> courseAEDict = new Dictionary<CourseRecord, HC.JHAEIncludeRecord>();
            ...
            foreach (var course in Course.Instance.Items)
            {
                ...
                foreach (HC.JHAEIncludeRecord ae in _aeCache.Values)
                {
                ...
                //    {
                //        HC.JHAEIncludeRecord ae2 = _aeCache[ae.ID];
                //        if (!courseAEDict.ContainsKey(course))
            ...
            Dictionary<string, List<HC.JHSCETakeRecord>> courseScoreDict = GetCourseScores(exam_id,CourseIDs);

            _courseListViewItemList.Clear();
            foreach (CourseRecord course in courseAEDict.Keys)
            {
                List<HC.JHSCETakeRecord> sceList = null;
                if (courseScoreDict.ContainsKey(course.ID))
                    sceList = courseScoreDict[course.ID];
                else
                    sceList = new List<HC.JHSCETakeRecord>();
        ...
        /// </summary>
        /// <returns></returns>
        private Dictionary<string, List<HC.JHSCETakeRecord>> GetCourseScores(string exam_id,List<string> CourseIDs)
        {
            #region 過濾非在校生

            //先把成績依 StudentID 區分，為了過濾掉非在校生。
            Dictionary<string, List<HC.JHSCETakeRecord>> studentScoreDict = new Dictionary<string, List<HC.JHSCETakeRecord>>();           
...
//            foreach (JHSchool.Data.JHSCETakeRecord jhsce in JHSchool.Data.JHSCETake.SelectByCourseAndExam("", exam_id))
            ...
            {
                HC.JHSCETakeRecord sce = new HC.JHSCETakeRecord(jhsce);
                if (!studentScoreDict.ContainsKey(sce.RefStudentID))
                    studentScoreDict.Add(sce.RefStudentID, new List<HC.JHSCETakeRecord>());
                studentScoreDict[sce.RefStudentID].Add(sce);
            }
            //foreach (var sce in JHSchool.Evaluation.Feature.QuerySCETake.GetHC.JHSCETakeRecords("", exam_id))
            //{
            //    if (!studentScoreDict.ContainsKey(sce.RefStudentID))
            //        studentScoreDict.Add(sce.RefStudentID, new List<HC.JHSCETakeRecord>());
            //    studentScoreDict[sce.RefStudentID].Add(sce);
            ...
            List<HC.JHSCETakeRecord> sceList = new List<HC.JHSCETakeRecord>();
            ...
            Dictionary<string, List<HC.JHSCETakeRecord>> courseScoreDict = new Dictionary<string, List<HC.JHSCETakeRecord>>();

            foreach (var sce in sceList)
            {
                if (!courseScoreDict.ContainsKey(sce.RefCourseID))
                    courseScoreDict.Add(sce.RefCourseID, new List<HC.JHSCETakeRecord>());
