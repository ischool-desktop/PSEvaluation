...
namespace JHSchool.Evaluation.StudentExtendControls.Ribbon.Ranking
{
    internal class CombineReport : AbstractReport
    {
        ...
        public CombineReport(List<RankData> rankData, Dictionary<string, RStudentRecord> studentDict)
        {
            _rankData = rankData;
        ...
        public override void Export()
        {
            ...
            foreach (RStudentRecord student in list)
            {
                ...
                foreach (string name in printData[student.ID].Keys)
                {
                    ...
                    ws.Cells[rowIndex, itemColIndex[name] + 1].PutValue("" + printData[student.ID][name].Rank+Display);
                }
