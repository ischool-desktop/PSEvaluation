...
{
    public partial class LearningDomainScoreCalculateByGradeyear : BaseForm, IStatusReporter
    {
        private List<string> StudentIDs;
        private int SchoolYear { get; set; }
        private int Semester { get; set; }

        public LearningDomainScoreCalculateByGradeyear()
        {
            InitializeComponent();
        }
        ...
        {
            DialogResult dr = MsgBox.Show("您確定要計算學生學期學習領域成績？", MessageBoxButtons.YesNo);
        ...
        private void worker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                ...
                logger.Log("學生學期學習領域成績計算(教務作業)", "計算學生學期學習領域成績", "計算學生數：" + Students.Count);
                ...
                foreach (StudentScore each in Students)
                {
                    ...
                    logger.AddBatch("學生學期學習領域成績計算(教務作業)", "計算學生學期學習領域成績", "student", each.Id, description.ToString());
                }
                logger.LogBatch();
            }
            catch { }
